...
<div class="grid grid-cols-2 grid-rows-2 gap-0">
    ...
    @else
    <div class="col-span-full bg-red-900  mx-auto min-w-full h-120 text-red-200 font-bold pt-15">
        <span class="block text-center text-8xl font-roboto">{{ __('Sorry, something was wrong') }}</span>
        <div class="block pt-15">
            <a href="/{{ app()->getLocale() }}/" class="block mx-auto underline text-center text-2xl font-poppins bg-white text-red-500 p-3 w-md rounded-xl">{{ __('return back to index') }}</a>
        </div>
    </div>
    @endif
